...
enum TokenKind {
    ...
    LONG("long"),
    NEW("new"), 
    PACKAGE("package"), PRIVATE("private"), PROTECTED("protected"), PUBLIC("public"), RETURN("return"), 
    STATIC("static"), SUPER("super"), SWITCH("switch"),
    THIS("this"), THROW("throw"), THROWS("throws"), TRY("try"),
    VOID("void"), 
    WHILE("while"),

    // Operators.
    ALS("<<"), ALS_ASSIGN("<<="), AND("&"), AND_ASSIGN("&="), ARS(">>"), ARS_ASSIGN(">>="), ASSIGN("="),
    COLON(":"), COMPL("~"),
    DEC("--"), DIV("/"), DIV_ASSIGN("/="), 
    EQUAL("=="), 
    GE(">="), GT(">"), 
    INC("++"), 
    LAND("&&"), LE("<="), LNOT("!"), LOR("||"), LRS(">>>"), LRS_ASSIGN(">>>="), LT("<"),
    MINUS("-"), MINUS_ASSIGN("-="),
    NE("!="),
    OR("|"), OR_ASSIGN("|="),
    PLUS("+"), PLUS_ASSIGN("+="), 
    ...
    COMMA(","), 
    DOT("."), 
    LBRACK("["), LCURLY("{"), LPAREN("("), 
    RBRACK("]"), RCURLY("}"), RPAREN(")"), 
    SEMI(";"),
