...
class Scanner {
    ...
    public TokenInfo getNextToken() {
                    ...
                    while (ch != EOFCH) {
                        if (ch == '*') {
                            ...
                            if (ch == '/') {
                                ...
                                // Still in the comment, keep reading
                                buffer.append(ch);
                                nextCh();
                            }
                        } else {
        ...
        switch (ch) {
            ...
            case '.':
                nextCh();
                // Double Literals - proj2.4
                if (isDigit(ch)) {
                    buffer = new StringBuffer();
                    buffer.append('.');
                    while (isDigit(ch)) {
                        // Keep reading if next character is a number/digit
                        buffer.append(ch);
                        nextCh();
                    }
                    if (ch == 'D' || ch == 'd' || ch == 'E' || ch == 'e') {
                        ...
                        // Add current ch (E/e) and also add +/- to buffer (ie. 3e+2, 4e-4)
                        if (ch == 'E' || ch == 'e') {
                            buffer.append(ch);
                            nextCh();
                        }
                        ...
                        // Then keep reading digits (ie. .02D)
                        while (isDigit(ch)) {
                            buffer.append(ch);
                            nextCh();
                        }
                    ...
                    // Just ends with the final state of the transition diagram (ie. .314)
                    return new TokenInfo(DOUBLE_LITERAL, buffer.toString(), line);
                } else
            ...
            case '9':
                ...
                if (ch == 'L' || ch == 'l') {
                    /* 
                     * Long literal - proj2.4 
                     * Digits ending with L/l makes it a long literal
                    */
                    buffer.append(ch);
                    nextCh();
                    ...
                    /*
                     * Double literal - proj2.4
                     * Digits starting with '.', 'E', 'e', 'D', 'd' makes it a double literal
                     */
                    buffer.append(ch);
                    nextCh();
                    // Add current ch (E/e) and also add +/- to buffer (ie. 3e+2, 4e-4)
